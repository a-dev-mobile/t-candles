...
impl TinkoffCandlesScheduler {
    ...
    pub async fn start(&self) {
        ...
        let candle_config = &self.app_state.settings.app_config.tinkoff_historical_candle_updater;
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let candle_config = &app_state.settings.app_config.tinkoff_historical_candle_updater;
                let operation_allowed = is_operation_allowed(candle_config);
                
                if !operation_allowed {
                    debug!("Candle scheduler: skipping update - outside operation window (current time: {})", 
                ...
                info!("Candle scheduler: triggering candle data update");
                
                // Trigger candle update
                match candle_client.load_and_save_candles().await {
                    Ok(count) => info!("Candle scheduler: successfully processed {} instruments", count),
...
fn is_operation_allowed(candle_config: &crate::env_config::models::app_config::TinkoffHistoricalCandleDataConfig) -> bool {
