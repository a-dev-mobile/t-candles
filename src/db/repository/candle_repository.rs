...
pub trait CandleRepository {
    async fn insert_candles(&self, candles: &[DbCandle]) -> Result<u64, ClickhouseError>;
    async fn get_candles_by_figi(
        &self,
        figi: &str,
        from: DateTime<Utc>,
        to: DateTime<Utc>,
    ) -> Result<Vec<DbCandle>, ClickhouseError>;
    async fn get_daily_candles_by_figi(
        &self,
        figi: &str,
        from: NaiveDate,
        to: NaiveDate,
    ) -> Result<Vec<DailyCandle>, ClickhouseError>;
}
...
impl CandleRepository for ClickhouseCandleRepository {
    ...
    async fn get_candles_by_figi(
        ...
        let candles = client.query(&query).fetch_all::<DbCandle>().await?;
        Ok(candles)
    }
