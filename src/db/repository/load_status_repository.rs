...
pub trait LoadStatusRepository {
    async fn insert_load_status(&self, status: &LoadStatus) -> Result<(), ClickhouseError>;
    async fn get_load_status_by_figi(&self, figi: &str) -> Result<Vec<LoadStatus>, ClickhouseError>;
    async fn get_latest_load_date_by_figi(&self, figi: &str) -> Result<Option<NaiveDate>, ClickhouseError>;
}
...
impl LoadStatusRepository for ClickhouseLoadStatusRepository {
    async fn insert_load_status(&self, status: &LoadStatus) -> Result<(), ClickhouseError> {
        ...
        info!("Inserted load status for figi {} from {} to {}", 
    ...
    async fn get_load_status_by_figi(&self, figi: &str) -> Result<Vec<LoadStatus>, ClickhouseError> {
        ...
        let result = client.query(&query).fetch_all::<LoadStatus>().await?;
        Ok(result)
    }
