...
pub trait TraitTinkoffCandlesStatusRepository {
    ...
    async fn get_by_instrument_uid(
        &self,
        instrument_uid: &str,
    ) -> Result<Option<TinkoffCandlesStatus>, SqlxError>;
...
impl TraitTinkoffCandlesStatusRepository for StructTinkoffCandlesStatusRepository {
    async fn get_by_instrument_uid(
        ...
        match &result {
            ...
            Err(e) => error!(
                "Error fetching status for instrument_uid {}: {}",
                instrument_uid, e
            ),
        }
    ...
    async fn upsert(
        ...
        debug!(
            "Upserting status for instrument_uid: {}, to_second: {}",
            instrument_uid, to_second
        );
        ...
        match &result {
            Ok(pg_result) => debug!(
                "Upserted status for instrument_uid: {}, rows affected: {}",
                instrument_uid,
                pg_result.rows_affected()
            ),
            Err(e) => error!(
                "Error upserting status for instrument_uid {}: {}",
                instrument_uid, e
            ),
        }
    ...
    async fn update_to_second(
        ...
        debug!(
            "Updating to_second for instrument_uid: {}, new value: {}",
            instrument_uid, to_second
        );
        ...
        match &result {
            Ok(pg_result) => {
                if pg_result.rows_affected() > 0 {
                    ...
                    debug!(
                        "No record found to update for instrument_uid: {}",
                        instrument_uid
                    );
                }
            }
            Err(e) => error!(
                "Error updating to_second for instrument_uid {}: {}",
                instrument_uid, e
            ),
        }
        ...
        match &result {
            Ok(pg_result) => {
                if pg_result.rows_affected() > 0 {
                    ...
                    debug!(
                        "No record found to delete for instrument_uid: {}",
                        instrument_uid
                    );
                }
            }
            Err(e) => error!(
                "Error deleting status for instrument_uid {}: {}",
                instrument_uid, e
            ),
        }
